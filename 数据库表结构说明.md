...
- `points` - DECIMAL(12,4) NOT NULL DEFAULT 0.0000（点数，支持4位小数）
...
- `withdrawable_balance` - BIGINT NOT NULL DEFAULT 0（可提现余额）
...
- `amount` - DECIMAL(12,2) NOT NULL（奖励金额）
- `order_id` - BIGINT UNSIGNED NOT NULL（订单ID）
...
- **主要业务模块：**
  ...
  - 订单模块：orders, order_items, refunds, alert_order
...
- **精度说明：**
  - 积分相关字段（points, merchant_points, change_amount, balance_after, points_before, points_deducted）支持小数点后4位精度
